...
class ChatClient(Frame):
  ...
  def initUI(self):
    ...
    serverSetButton = Button(ipGroup, text="Start Server", width=10, command=self.handleSetServer)
    ...
    self.receivedChats = Text(readChatGroup, bg="grey", width=60, height=30, state=DISABLED)
    self.friends = Listbox(readChatGroup, bg="grey", width=30, height=30)
  ...
  def handleSetServer(self):
    ...
    try:
        ...
        self.setStatus("Error starting server")
  ...
  def handleAddClient(self):
    if self.serverStatus == 0:
      self.setStatus("start server first")
      return
